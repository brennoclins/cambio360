    ...
    <div className="p-5 space-y-3 animate-pulse">
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="flex gap-6 h-5">
          <div className="w-20 h-full bg-slate-200 rounded" />
          <div className="w-16 h-full bg-slate-200 rounded" />
          <div className="w-16 h-full bg-slate-200 rounded" />
          <div className="w-14 h-full bg-slate-200 rounded" />
          <div className="w-14 h-full bg-slate-200 rounded" />
        </div>
      ))}
    </div>
...
export function HistoryTable({ history, loading, error, pair }: HistoryTableProps) {
  return (
    <section className="bg-white rounded-2xl ring-1 ring-slate-200/60 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Últimos 15 dias &middot; {pair}
            </p>
          </div>
          <div className="flex items-center gap-1.5 text-[10px] text-slate-400 uppercase tracking-wider bg-slate-50 px-2 py-1 rounded-md">
      ...
      <div className="overflow-x-auto">
        {loading ? (
          ...
          <div className="flex items-center justify-center gap-2 p-8 text-sm text-red-500">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Compra</th>
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Venda</th>
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Variação</th>
                <th className="text-right px-5 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">%</th>
              </tr>
            </thead>
              ...
              {history.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right font-medium tabular-nums text-slate-800">
                      {item.ask ? formatBRL(item.ask) : <span className="text-slate-300">&mdash;</span>}
                    </td>
