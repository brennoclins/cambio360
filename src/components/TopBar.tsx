...
export function TopBar({ lastUpdated, refreshing, onRefresh, onMenuToggle }: TopBarProps) {
  return (
    <header className="sticky top-0 z-20 bg-[#0c1a16]/80 backdrop-blur-xl border-b border-white/5">
      <div className="flex items-center justify-between px-4 sm:px-6 py-3">
        <div className="flex items-center gap-3">
          <button
            onClick={onMenuToggle}
            className="lg:hidden p-2 rounded-lg hover:bg-white/5 text-slate-500 transition-colors cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-[#a3e635]/10 rounded-lg ring-1 ring-[#a3e635]/20">
            <span className={`inline-block w-2 h-2 rounded-full ${refreshing ? 'bg-[#a3e635] animate-pulse' : 'bg-[#a3e635]'}`} />
            <span className="text-xs text-[#a3e635] font-medium">
          ...
          <button
            ...
            onClick={onRefresh}
            disabled={refreshing}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[#0a1410] bg-[#a3e635] rounded-lg
                       hover:bg-[#84cc16] transition-all cursor-pointer
